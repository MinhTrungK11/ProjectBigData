 ...
 * @author Trung
 */
@WebServlet(name = "SearchByAuthor", urlPatterns = {"/searchByAuthor"})
public class SearchByAuthor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        DAO dao = new DAO();
        List<Books_By_Author> book;
        List<Books_By_Title> books;
        String author = request.getParameter("searchauthor");
        
        String optionsearch = request.getParameter("optionsearch");
        
        if (optionsearch.equals("Author")) {
            book = dao.get1Book_By_Author(author);
            request.setAttribute("Listbook", book);
        } else {
            books = dao.get1Book_By_Title(author);  
